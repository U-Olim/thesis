...
gmm<-function(y,D,X,Z,tau,alphaint,core){
    ...
    A=foreach(i = alphaint, .combine = "rbind") %dopar%{
        ...
        distribition=c(dnorm(e,mean(e),var(e)))   #(8)??????Kernal?A?]????Kernal?w???t
...
hdm_crossfit<-function(y,D,X,Z,tau,alphaint,crossfold,core,l1_norm=TRUE,POST=TRUE){
    ...
    for (cf in 1:crossfold) {
        ...
        A=foreach(i = alphaint, .combine = "rbind") %dopar%{
            ...
            distribition=sqrt(diag(c(dnorm(e_in,mean(e_in),var(e_in)))))   #(8)??????Kernal?A?]????Kernal?w???t
...
overall_ptm <- proc.time()

checkpoint_file <- paste0("checkpoint_est_", sample_size, ".rds")
start_it <- 1
if (file.exists(checkpoint_file)) {
    est <- readRDS(checkpoint_file)
    start_it <- nrow(est) + 1
    cat("Resuming from checkpoint: replication", start_it, "of", iter, "\n")
}

if (start_it > iter) {
    stop("Checkpoint already contains ", nrow(est), " replications (iter = ", iter,
         "). Delete ", checkpoint_file, " to start a fresh run.")
}

for (it in start_it:iter) {
        ptm <- proc.time()
    success <- FALSE
    attempt <- 0
    while (!success) {
        attempt <- attempt + 1
        if (attempt > 20) {
            stop("Replication ", it, " failed 20 times in a row - stopping. Progress is saved in ", checkpoint_file)
        }
        q <- tryCatch({
            n = sample_size
            p = 100
            ...
            cbind(exact_q25,exact_q50,exact_q75,qr_q25,qr_q50,qr_q75,
                  qr_cv_q25,qr_cv_q50,qr_cv_q75)
        }, error = function(e) {
            cat("Replication", it, "attempt", attempt, "failed:", conditionMessage(e), "- retrying with a fresh draw\n")
            return(NULL)
        })
        if (!is.null(q)) {
            success <- TRUE
    ...
    new_est=q
    if(it == start_it){
        if (start_it == 1) { est = new_est } else { est = rbind(est, new_est) }
    }
    else{
    ...
    if (it %% 10 == 0 || it == iter) {
        saveRDS(est, checkpoint_file)
        cat("Checkpoint saved at replication", it, "of", iter, "\n")
    }
}
