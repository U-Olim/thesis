...
# --- Profile versions: return the FULL alpha grid + GMM(alpha) values, not just the minimizer ---

gmm_quantile_profile <- function(y, D, X, Z, tau) {
  alpha = seq(-1, 3, length = 41)
  gmm = rep(0, length(alpha))
  for (i in 1:length(alpha)) {
    beta <- rq(y - (alpha[i]*D) ~ X, tau = tau)
  ...
  return(list(alpha = alpha, WN = gmm))
}

hdm_quantile_profile <- function(y, D, X, Z, tau) {
  alpha = seq(-1, 3, length = 41)
  gmm = rep(0, length(alpha))
  for (i in 1:length(alpha)) {
    lasso = cv.hqreg(X, y - alpha[i]*D, method = c("quantile"), tau = tau,
                     FUN = c("hqreg"), nfolds = 5, type.measure = c("mae"))
    cv.beta = as.matrix(lasso$fit$beta)
